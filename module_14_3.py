...
buy_but = []
for i in range(4):
    buy_but.append(InlineKeyboardButton(text=f'Product{i+1}', callback_data='product_buying'))
buy_kb.row(*buy_but)
...
async def get_buying_list(message):
    for prod in range(1, 5):
        with open(f'product{prod}.jpg', 'rb') as img:
            await message.answer_photo(img, f'Название: Product{prod} | Описание: описание {prod} | Цена: {prod * 100}')
